...
def monthToInt(abbr_month):
    'converts abbreviated month to integer representation example: DEC -> 12'
    months = [month.upper() for month in calendar.month_abbr]
    return months.index(abbr_month)


def getDate(soup):
    '''input: soup from RB review page
    output: list of tuples of month, day, year for each review'''
    out = []
    tab = soup.findAll('table')
    small = tab[2].findAll('small')
    for r in small:
        if '-' in r.text:
            d = r.text.split('-')[-1].split(',')
            for i, x in enumerate(d[::2]):
                month =  monthToInt(x[1:4])
                day = int(x[4:])
                year = int(d[i+1][1:5])
            out.append((month, day, year))
        if 'UPDATED' in r.text:
            d = r.text.split(' ')
            month = monthToInt(d[1])
            day = int(d[2][:-1])
            year = int(d[3])
            out[-1] = (month,day,year)
    return out

def getState(soup):
    state = u'none'
    div = soup.find('div', style="padding-bottom: 7px; line-height: 1.5;")
    links = div.findAll('a')
    for a in links:
        if '/places/' in a['href']:
            state = a.text.strip()
...
def collectReviews(beer_url):
    ''' creates a list of dicts for all reviews of given beer
        ...
        output: json style dict of review data'''
    out = []
    try:
        soup_list = collectReviewSoup(beer_url)
    except:
        logging.error(beer_url)
